...
## HNS support (automatic)

Polaris automatically includes `roles/storage.folderAdmin` in the access boundary rules for all GCS write operations. This means credential vending will always grant the permissions needed to create managed folders on write paths, using scoped `resource.name.startsWith(...)` conditions.

If your GCS bucket has [Hierarchical Namespace (HNS)](https://docs.cloud.google.com/storage/docs/hns-overview) enabled, no additional configuration is required. The `roles/storage.folderAdmin` permission allows the scoped (vended) token to create managed folders, which HNS buckets require for path operations.

For non-HNS buckets, the additional `roles/storage.folderAdmin` permission is harmless and has no effect on bucket behavior.
