...
public class GcsStorageLocationPreparer implements StorageLocationPreparer {
  ...
  static final int HNS_CACHE_MAX_SIZE = 1_000;
  static final Duration HNS_CACHE_TTL = Duration.ofMinutes(30);

  private static final Cache<String, Boolean> HNS_STATUS_CACHE =
      Caffeine.newBuilder()
          .maximumSize(HNS_CACHE_MAX_SIZE)
          .expireAfterWrite(HNS_CACHE_TTL)
          .build();
  ...
  private boolean resolveHnsStatus(String bucketName) {
    return HNS_STATUS_CACHE.get(bucketName, this::queryBucketHnsStatus);
  }
  ...
  /** Clears the cached HNS status for all buckets. Intended for testing. */
  public static void clearHnsCache() {
    HNS_STATUS_CACHE.invalidateAll();
  }
}
